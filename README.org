...
bits in a canonical form with certain constraints and using a
certain structures.
...
this for each harware sized quantity in which the bit pattern is
stored. In Common Lisp, with arbitrary sized integers, there is more
work to accomodate sign extension rules.

The structure in question is the ordering of the multiple bytes
(here, defined here as M N-bit unsigned chunks). This is the usual
understanding of *Byte Endianess* with respect to integers.
