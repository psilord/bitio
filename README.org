...
NOTE: This function doesn't understand properties of integers. It
only reads bits from the underlying octet stream. Under certain
conditions, it is logical to interpret the numbers this function
returns as unsigned integers, but in general, if you want to read
integers explicitly, there is a function for that described later.
