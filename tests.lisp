...
;; TODO, maybe make this duplicate the test from a file as well.
...
(defun doit ()
  (let ((octet-vector (make-octet-vector)))
    ...
    (run-tests/read-bits octet-vector)
    (run-tests/read-one-byte octet-vector)
    (run-tests/read-bytes octet-vector)
    (run-tests/read-integer octet-vector)
    ...
    (format t "Case: wrapping fast-io 1 octet stream from a file~%")
    (with-open-file (fin (asdf:system-relative-pathname :bitio "binfile")
                         :direction :input
                         :element-type '(unsigned-byte 8)
                         :if-does-not-exist :error)
      (fast-io:with-fast-input (fin-fast
        ...
        (let ((bitio (test-make-bitio/fast-io fin-fast)))
          (test-read-bits bitio 32 :be #x00010203)
          )))
...
(defun run-tests/read-bits (octet-vector)
  ...
  (do-test :fast-io 4096 octet-vector bitio "READ-BITS"
      "all aligned read, octet buffer size larger, intended eof, bit :be"
    ;; Consume 10 legal octets
    (test-read-bits bitio 80 :be #x5cf6ee799adefff28802)
    ;; Here we expect to read an eof.
    (test-read-bits bitio 8 :be :eof 0 NIL :eof))

  (do-test :fast-io 4096 octet-vector bitio "READ-BITS"
      "all aligned read, octet buffer size larger, unintended eof, bit :be"
  ...
  (do-test :fast-io 7 octet-vector bitio "READ-BITS"
      "all aligned read, octet buffer size smaller, unintended eof, bit :be"
    ;; Consume 10 legal octets, and then one more (80 bits plus 8
    ;; bits) resulting in EOF.  Expected value here is the
    ;; expected truncated return.
    (test-read-bits bitio 88 :be #x5cf6ee799adefff28802 80 NIL :eof)
  ...
  (do-test :fast-io 1 octet-vector bitio "READ-BITS"
      "all aligned read, octet buffer size 1, unintended eof, bit :be"
    ;; Consume 10 legal octets, and then one more (80 bits plus 8
    ;; bits) resulting in EOF.  Expected value here is the
    ;; expected truncated return.
    (test-read-bits bitio 88 :be #x5cf6ee799adefff28802 80 NIL :eof)
  ...
  (do-test :fast-io 4096 octet-vector bitio "READ-BITS"
      "mixed w/inferred aligned read, octet buffer larger, unintended eof, bit :be"
    ;; Consume .5 octet
    (test-read-bits bitio 4 :be #x5)
  ...
  (do-test :fast-io 4096 octet-vector bitio "READ-BITS"
      "mixed w/inferred aligned read, octet buffer larger, intended eof after stable read, bit :be"

    ;; Consume .5 octet, 9 legal octets. This leaves 4 bits in read stable.
...
(defun run-tests/read-one-byte (octet-vector)
...
(defun run-tests/read-bytes (octet-vector)
...
(defun run-tests/read-integer (octet-vector)
